import{_ as e}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as i,o as n,d as s}from"./app-CMzW_-C7.js";const a="/assets/1034759-20191123175410495-632059730-16918406651821-zxJIbfm2.png",d="/assets/1034759-20191123195126520-796246549-16918406692903-C-jYa0mg.png",r={},t=s('<h1 id="kubernetes进阶-四-服务暴露-ingress控制器之traefik" tabindex="-1"><a class="header-anchor" href="#kubernetes进阶-四-服务暴露-ingress控制器之traefik"><span>kubernetes进阶（四）服务暴露-ingress控制器之traefik</span></a></h1><p>上一章我们测试了在集群内部解析service名称，</p><p>下面我们测试在集群外部解析：</p><figure><img src="'+a+`" alt="img" tabindex="0" loading="lazy"><figcaption>img</figcaption></figure><p>根本解析不到，因为我们外部用的dns是10.4.7.11，也就是我们的自建bind dns，这个DNS服务器上也没有响应的搜索域。</p><p>如何能让集群外部访问nginx-dp？</p><p>这里有两种服务暴露方式：修改工作模式，在kube-proxy中修改，并重启</p><p>1、使用nodeport方式，但是这种方式不能使用ipvs，只能使用iptables，iptables只能使用rr调度方式。原理相当于端口映射，将容器内的端口映射到宿主机上的某个端口。</p><p>2、使用ingress，但是只能工作在七层网络下，建议暴露http, https可以使用前端nginx来做证书方面的卸载 ---推荐使用</p><p>Ingress是基于域名和URL路径，将用户的请求转发至特定的service资源。</p><p>下面我们部署traefik：<a href="https://github.com/containous/traefik" target="_blank" rel="noopener noreferrer">GITHUB官方地址</a> 在hdss7-200上执行：</p><p>下载镜像：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code># docker pull traefik:v1.7.2-alpine
