import{_ as o}from"./plugin-vue_export-helper-c27b6911.js";import{o as i,c as n,f as r}from"./app-f2f14b0c.js";const e={},t=r(`<h1 id="grub2各种手动命令引导教程" tabindex="-1"><a class="header-anchor" href="#grub2各种手动命令引导教程" aria-hidden="true">#</a> grub2各种手动命令引导教程</h1><h2 id="手动引导ubuntu的iso镜像文件" tabindex="-1"><a class="header-anchor" href="#手动引导ubuntu的iso镜像文件" aria-hidden="true">#</a> 手动引导ubuntu的iso镜像文件</h2><p>从而安装ubuntu，grub&gt;代表命令的开始</p><p>假设ubuntu镜像在U盘的第一个分区的根目录下即：(hd0,1)/ubuntu-18.04-desktop-amd64.iso</p><p>手动引导下可以按TAB键补全命令、目录以及文件名</p><pre><code>#查询所有已安装磁盘并打印详细信息
grub&gt;ls -l
...
#根据上面的参数启动系统
grub&gt;boot
</code></pre><p>-----------------------------------------------------------------------分割线-----------------------------------------------------------------------------------------</p><h2 id="手动引导archlinux的iso镜像文件从而安装archlinux" tabindex="-1"><a class="header-anchor" href="#手动引导archlinux的iso镜像文件从而安装archlinux" aria-hidden="true">#</a> 手动引导archlinux的iso镜像文件从而安装archlinux</h2><p>，grub&gt;代表命令的开始</p><p>假设archlinux镜像在U盘的第一个分区的根目录下即：(hd0,1)/archlinux-2018.07.01-x86_64.iso</p><pre><code>grub&gt;set root=(hd0,1)
...
grub&gt;initrd (loop)/arch/boot/x86_64/archiso.img
</code></pre><p>-----------------------------------------------------------------------分割线-----------------------------------------------------------------------------------------</p><h2 id="手动引导ubuntu" tabindex="-1"><a class="header-anchor" href="#手动引导ubuntu" aria-hidden="true">#</a> 手动引导Ubuntu</h2><p>假设ubuntu安装在第一块硬盘的第一个分区即：(hd0,1)</p><pre><code>grub&gt;root=(hd0,1)
grub&gt;linux /boot/vmlinuz-xxx
grub&gt;initrd /boot/initrd.img-xxx
grub&gt;boot
</code></pre><p>XXX是内核版本号</p><p>关于linux的通用引导方法其实与上面差不多，就是在“grub&gt;linux /boot/vmlinuz-xxx”这一句加载内核的文件与后面跟的参数有所不同，不同的linux发行版会有不同的参数，有些可以也不加参数启动</p><p>-----------------------------------------------------------------------分割线-----------------------------------------------------------------------------------------</p><h2 id="手动引导uefi启动模式下的windows。" tabindex="-1"><a class="header-anchor" href="#手动引导uefi启动模式下的windows。" aria-hidden="true">#</a> 手动引导uefi启动模式下的Windows。</h2><p>winPE也可以用此方法引导</p><p>假设windows安装在第一块硬盘的第一个分区即：(hd0,1)</p><pre><code>#加载ntfs文件系统
grub&gt;insmod ntfs
grub&gt;set root=(hd0,1)
...
grub&gt;boot
</code></pre><p>-----------------------------------------------------------------------分割线-----------------------------------------------------------------------------------------</p><h2 id="手动引导bios启动模式下的windows。" tabindex="-1"><a class="header-anchor" href="#手动引导bios启动模式下的windows。" aria-hidden="true">#</a> 手动引导bios启动模式下的Windows。</h2><p>winPE也可以用此方法引导</p><p>假设windows安装在第一块硬盘的第一个分区即：(hd0,1)</p><p>chainloader /bootmgr 命令会报签名错误，即使关闭签名验证也无法启动(chainloader +1也可启动系统但不可启动U盘pe)</p><pre><code>grub&gt;set root=(hd0,1)
...
grub&gt;ntldr /bootmgr
grub&gt;boot
</code></pre>`,30),u=[t];function a(s,d){return i(),n("div",null,u)}const l=o(e,[["render",a],["__file","grub2手动命令引导教程.html.vue"]]);export{l as default};
