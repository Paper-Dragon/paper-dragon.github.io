...
kind: ClusterStatus
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可也看到 <code>k8s-master02</code> 节点信息还存在与 <code>kubeadm</code> 配置中，说明 <code>etcd</code> 中还存储着 <code>k8s-master02</code> 相关信息。</p><h3 id="_3-分析问题所在及解决方案" tabindex="-1"><a class="header-anchor" href="#_3-分析问题所在及解决方案"><span>3 分析问题所在及解决方案</span></a></h3><p>因为集群是通过 <code>kubeadm</code> 工具搭建的，且使用了 <code>etcd</code> 镜像方式与 <code>master</code> 节点一起，所以每个 <code>Master</code> 节点上都会存在一个 <code>etcd</code> 容器实例。当剔除一个 <code>master</code> 节点时 <code>etcd</code> 集群未删除剔除的节点的 <code>etcd</code> 成员信息，该信息还存在 <code>etcd</code> 集群列表中。</p><p>所以，我们需要 <strong>进入 etcd 手动删除 etcd 成员信息</strong>。</p><h2 id="三-解决问题" tabindex="-1"><a class="header-anchor" href="#三-解决问题"><span>三 解决问题</span></a></h2><h3 id="_1-获取-etcd-镜像列表" tabindex="-1"><a class="header-anchor" href="#_1-获取-etcd-镜像列表"><span>1 获取 Etcd 镜像列表</span></a></h3><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>[root@k8s-master01 ~]# kubectl get pods -n kube-system | grep etcd
etcd-k8s-master01                         1/1     Running            4          4d20h
etcd-k8s-master03                         1/1     Running            1          4d20h
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_2-进入-etcd-容器并删除节点信息" tabindex="-1"><a class="header-anchor" href="#_2-进入-etcd-容器并删除节点信息"><span>2 进入 Etcd 容器并删除节点信息</span></a></h3><p>选择上面两个 etcd 中任意一个 pod，通过 kubectl 工具进入 pod 内部</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>[root@k8s-master01 ~]# kubectl exec -it -n kube-system etcd-k8s-master01 sh
kubectl exec [POD] [COMMAND] is DEPRECATED and will be removed in a future version. Use kubectl kubectl exec [POD] -- [COMMAND] instead.
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>进入容器后，按下面步执行</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>## 配置环境
...
## 退出容器
# exit
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_3-再次尝试加入集群" tabindex="-1"><a class="header-anchor" href="#_3-再次尝试加入集群"><span>3 再次尝试加入集群</span></a></h3><p>通过 <code>kubeadm</code> 命令再次尝试将 <code>k8s-master02</code> 节点加入集群，在执行前首先进入到 <code>k8s-master02</code> 节点服务器，执行 <code>kubeadm</code> 的清除命令：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>$ kubeadm reset
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>然后尝试加入 kubernetes 集群：</p><div class="language-text line-numbers-mode" data-ext="text" data-title="text"><pre class="language-text"><code>[check-etcd] Checking that the etcd cluster is healthy
...
k8s-master03   Ready    master   4d20h   v1.18.2
k8s-node01     Ready    worker   4d18h   v1.18.2
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,33),s=[a];function c(r,l){return t(),n("div",null,s)}const m=e(i,[["render",c],["__file","一次kubeadm添加节点出现etcd检查错误.html.vue"]]),v=JSON.parse('{"path":"/note-book/Kubernetes/%E4%B8%80%E6%AC%A1kubeadm%E6%B7%BB%E5%8A%A0%E8%8A%82%E7%82%B9%E5%87%BA%E7%8E%B0etcd%E6%A3%80%E6%9F%A5%E9%94%99%E8%AF%AF.html","title":"一次kubeadm添加节点出现etcd检查错误","lang":"zh-CN","frontmatter":{"description":"一次kubeadm添加节点出现etcd检查错误 错误关键字 执行 kubeadm join... 时 [check-etcd] Checking that the etcd cluster is healthy error execution phase check-etcd: etcd cluster is not healthy: failed t...","head":[["meta",{"property":"og:url","content":"https://paper-dragon.github.io/note-book/Kubernetes/%E4%B8%80%E6%AC%A1kubeadm%E6%B7%BB%E5%8A%A0%E8%8A%82%E7%82%B9%E5%87%BA%E7%8E%B0etcd%E6%A3%80%E6%9F%A5%E9%94%99%E8%AF%AF.html"}],["meta",{"property":"og:site_name","content":"运维开发绿皮书"}],["meta",{"property":"og:title","content":"一次kubeadm添加节点出现etcd检查错误"}],["meta",{"property":"og:description","content":"一次kubeadm添加节点出现etcd检查错误 错误关键字 执行 kubeadm join... 时 [check-etcd] Checking that the etcd cluster is healthy error execution phase check-etcd: etcd cluster is not healthy: failed t..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2023-08-13T15:08:38.000Z"}],["meta",{"property":"article:author","content":"PaperDragon"}],["meta",{"property":"article:modified_time","content":"2023-08-13T15:08:38.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"一次kubeadm添加节点出现etcd检查错误\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2023-08-13T15:08:38.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"PaperDragon\\",\\"url\\":\\"https://github.com/Paper-Dragon\\",\\"email\\":\\"2678885646@qq.com\\"}]}"]]},"headers":[{"level":2,"title":"一 问题描述","slug":"一-问题描述","link":"#一-问题描述","children":[]},{"level":2,"title":"二 分析问题","slug":"二-分析问题","link":"#二-分析问题","children":[{"level":3,"title":"1 查看集群节点列表","slug":"_1-查看集群节点列表","link":"#_1-查看集群节点列表","children":[]},{"level":3,"title":"2 查看 Kubeadm 配置信息","slug":"_2-查看-kubeadm-配置信息","link":"#_2-查看-kubeadm-配置信息","children":[]},{"level":3,"title":"3 分析问题所在及解决方案","slug":"_3-分析问题所在及解决方案","link":"#_3-分析问题所在及解决方案","children":[]}]},{"level":2,"title":"三 解决问题","slug":"三-解决问题","link":"#三-解决问题","children":[{"level":3,"title":"1 获取 Etcd 镜像列表","slug":"_1-获取-etcd-镜像列表","link":"#_1-获取-etcd-镜像列表","children":[]},{"level":3,"title":"2 进入 Etcd 容器并删除节点信息","slug":"_2-进入-etcd-容器并删除节点信息","link":"#_2-进入-etcd-容器并删除节点信息","children":[]},{"level":3,"title":"3 再次尝试加入集群","slug":"_3-再次尝试加入集群","link":"#_3-再次尝试加入集群","children":[]}]}],"git":{"createdTime":1691939318000,"updatedTime":1691939318000,"contributors":[{"name":"PaperDragon","email":"2678885646@qq.com","commits":1}]},"readingTime":{"minutes":3.92,"words":1175},"filePathRelative":"note-book/Kubernetes/一次kubeadm添加节点出现etcd检查错误.md","localizedDate":"2023年8月13日","excerpt":"\\n<blockquote>\\n<p>错误关键字</p>\\n<p>执行 kubeadm join... 时</p>\\n<p>[check-etcd] Checking that the etcd cluster is healthy</p>\\n<p>error execution phase check-etcd: etcd cluster is not healthy: failed to dial endpoint https://10.8.18.105:2379 with maintenance client: context deadline exceeded To see the stack trace of this error execute with --v=5 or higher</p>\\n</blockquote>","autoDesc":true}');export{m as comp,v as data};
